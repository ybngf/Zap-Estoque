...
const Sidebar: React.FC<SidebarProps> = ({ view, setView, hasPermission, isOpen, onClose }) => {
  const [systemName, setSystemName] = React.useState<string>('Estoque Gemini');
  ...
  return (
    ...
    `}>
      ...
      <div className="h-20 flex items-center justify-center text-2xl font-bold text-white px-4">
        {logoUrl ? (
          <img src={logoUrl} alt={systemName} className="h-12 object-contain" onError={(e) => {
            (e.target as HTMLImageElement).style.display = 'none';
          }} />
        ) : (
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">{systemName}</span>
        )}
      </div>
